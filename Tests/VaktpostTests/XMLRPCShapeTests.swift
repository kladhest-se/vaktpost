...
/// UPnP mappings, whose field names are still not certain.
final class UPnPShapeTests: XCTestCase {
    ...
    func testTheAccessorsFieldNamesAreAccepted() throws {
        // What `upnp_get_active_mappings` is expected to return.
        let m = UPnPMapping(try dict("""
        {"proto": "UDP", "eport": "9308", "iaddr": "172.16.1.164",
         "iport": "9308", "interface": "wan", "descr": "9308 to 9308 (UDP)"}
        """))
        XCTAssertEqual(m.proto, "UDP")
        XCTAssertEqual(m.route, "9308 → 172.16.1.164:9308")
        XCTAssertEqual(m.interfaceName, "wan")
    }

    func testTheOtherNamingIsAlsoAccepted() throws {
        // The package's field names have changed between versions, and this
        // one is undocumented — so both spellings are read rather than
        // committing to the guess that was already wrong once.
        let m = UPnPMapping(try dict("""
        {"protocol": "udp", "external_port": "3074",
         "internal_address": "172.16.1.90", "internal_port": "3074"}
        """))
        XCTAssertEqual(m.proto, "UDP")
        XCTAssertEqual(m.route, "3074 → 172.16.1.90:3074")
    }

    func testAMissingDescriptionIsNilNotEmpty() throws {
        let m = UPnPMapping(try dict(#"{"proto": "TCP", "eport": "80"}"#))
        XCTAssertNil(m.descr)
    }

    func testTheSnippetReportsWhetherMappingsCouldBeRead() {
        // The flag the screen depends on. Without it an unreadable list and an
        // empty one are the same thing on screen, and they mean opposite
        // things.
        let script = PHPSnippet.upnp.script
        XCTAssertTrue(script.contains("mappings_readable"))
        // Still guarded, so a package version that gains the accessor starts
        // returning rows without another change.
        XCTAssertTrue(script.contains("function_exists(\"upnp_get_active_mappings\")"))
        // And no lease file: that path was guessed twice and does not exist.
        XCTAssertFalse(script.contains("miniupnpd.leases"))
    }
}

extension UPnPShapeTests {

    func testAnUnreadableMappingListIsNotAnEmptyOne() throws {
        // The distinction the screen exists to make. "No ports open" and
        // "cannot see which ports are open" render identically as an empty
        // list, and the first is far more comforting than it deserves to be.
        let status = UPnPStatus(try dict("""
        {"installed": true, "enabled": true, "running": true,
         "mappings_readable": false, "external_interface": "wan", "data": []}
        """))
        XCTAssertFalse(status.mappingsReadable)
        XCTAssertTrue(status.mappings.isEmpty)
        XCTAssertEqual(status.health, .warn)
    }

    func testDisabledIsTheSafeState() throws {
        let status = UPnPStatus(try dict("""
        {"installed": true, "enabled": false, "running": false,
         "mappings_readable": false, "data": []}
        """))
        XCTAssertEqual(status.health, .ok)
        XCTAssertNil(status.externalInterface)
    }

    func testAFutureAccessorWouldJustWork() throws {
        // The snippet still calls upnp_get_active_mappings where it exists, so
        // a package version that gains one starts returning rows with no other
        // change.
        let status = UPnPStatus(try dict("""
        {"installed": true, "enabled": true, "running": true,
         "mappings_readable": true, "data": [
           {"proto": "UDP", "eport": "9308", "iaddr": "172.16.1.164",
            "iport": "9308", "interface": "wan"}]}
        """))
        XCTAssertTrue(status.mappingsReadable)
        XCTAssertEqual(status.mappings.count, 1)
        XCTAssertEqual(status.mappings[0].route, "9308 → 172.16.1.164:9308")
    }
}
